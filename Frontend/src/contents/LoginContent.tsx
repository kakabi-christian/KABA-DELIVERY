...
export default function LoginContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true); // Correction apportée ici : loading(true) -> setLoading(true)
    setMessage(null);

    try {
      const response = await authService.login(formData);
      
      // Adaptation des redirections selon les rôles du projet CAMWATER
      ...
      <div className="login-card p-4 p-md-5 shadow-2xl bg-white rounded-5 border-0" 
        ...
        <div className="text-center mb-5" data-aos="fade-down" data-aos-delay="200">
          ...
          <p style={{ color: '#8A9BB0' }}>
            Accédez à votre espace abonné <span className="fw-bold" style={{ color: '#FF6B2B' }}>CAMWATER</span>
          </p>
        </div>
